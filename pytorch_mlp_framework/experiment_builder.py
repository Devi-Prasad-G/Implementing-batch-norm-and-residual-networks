...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
                 test_data, weight_decay_coefficient, lr , use_gpu, continue_from_epoch=-1):
        """
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        """
        ########################################
        for layer_name, parameters in named_parameters:
            if "weight" in layer_name:
                layer = layer_name.split(".")
                #print(layer)
                layers.append(layer[1]+"_"+layer[-2])
                all_grads.append(abs(parameters.grad).mean())
